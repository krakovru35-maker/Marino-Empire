...
test('manifest marks generated sprite sizes and project ownership accurately',()=>{for(const [id,file] of [['marino_building_icons',files.buildings],['marino_task_icons',files.tasks],['marino_casino_icons',files.casino]]){const asset=manifest.assets.find(item=>item.id===id);assert.equal(asset.available,true);assert.equal(asset.license,'Project-owned original');assert.equal(asset.sizeBytes,fs.statSync(file).size)}});

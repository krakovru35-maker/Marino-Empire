(() => {
  ...
  const ASSETS = {
    ...
    chip_place:{file:'./assets/audio/sfx/chip_place.ogg',available:false,fallback:[520,.04,'triangle'],rateLimit:45},chip_remove:{file:'./assets/audio/sfx/chip_remove.ogg',available:false,fallback:[330,.04,'sine'],rateLimit:45},wheel_start:{file:'./assets/audio/sfx/wheel_start.ogg',available:false,fallback:[210,.14,'sawtooth']},ball_tick:{file:'./assets/audio/sfx/ball_tick.ogg',available:false,fallback:[760,.025,'triangle'],rateLimit:55},ball_drop:{file:'./assets/audio/sfx/ball_drop.ogg',available:false,fallback:[420,.11,'triangle']},roulette_win:{file:'./assets/audio/sfx/roulette_win.ogg',available:false,fallback:[880,.2,'triangle']},roulette_lose:{file:'./assets/audio/sfx/roulette_lose.ogg',available:false,fallback:[260,.09,'sine']}
  };
  ...
  function procedural(definition){
    ...
    oscillator.type=wave;oscillator.frequency.value=frequency;gain.gain.setValueAtTime(lite()?.018:.032,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+duration);oscillator.connect(gain);gain.connect(ctx.destination);oscillator.onended=()=>{state.voices=Math.max(0,state.voices-1)};oscillator.start();oscillator.stop(ctx.currentTime+duration+.02);return true;
  }

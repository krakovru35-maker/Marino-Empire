(() => {
  ...
  if(typeof window!=='undefined'){window.MarinoCasinoMastery=Object.freeze({summaryMarkup,sheetMarkup,refresh,snapshot:()=>({points:{...state.points},badges:[...state.badges.keys()],selectedBadge:state.selectedBadge})});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind,{once:true});else bind();}

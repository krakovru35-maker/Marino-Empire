(() => {
  ...
  function render(){
    ...
    root.querySelector('[data-blackjack="double"]').disabled=!playerTurn||!canDouble(hand);root.querySelector('[data-blackjack="split"]').disabled=!playerTurn||!canSplit(hand)||hands.length>=4;
  ...
  function deal(){
    ...
    if(!useFree&&!window.MarinoDemoWallet?.debit?.(bet))return message('Demo jeton yetersiz.');
  ...
  function hit(){if(phase!==STATES.PLAYER_TURN||paused)return false;window.MarinoAudio?.play?.('hit');drawTo(active().cards);haptic('soft');const value=handValue(active().cards);if(value.bust){active().done=true;window.MarinoAudio?.play?.('bust');haptic('light');return nextHand();}if(value.total===21){active().done=true;return nextHand();}render();return true;}
  function stand(){if(phase!==STATES.PLAYER_TURN||paused)return false;window.MarinoAudio?.play?.('stand');active().done=true;return nextHand();}
  function doubleDown(){const hand=active();if(phase!==STATES.PLAYER_TURN||paused||!canDouble(hand)||!window.MarinoDemoWallet?.debit?.(hand.bet))return false;window.MarinoAudio?.play?.('double');hand.bet*=2;drawTo(hand.cards);hand.done=true;haptic('medium');return nextHand();}
  function split(){const hand=active();if(phase!==STATES.PLAYER_TURN||paused||!canSplit(hand)||hands.length>=4||!window.MarinoDemoWallet?.debit?.(hand.bet))return false;window.MarinoAudio?.play?.('split');const second={cards:[hand.cards.pop()],bet:hand.bet,done:false,fromSplit:true,free:false};hand.fromSplit=true;hands.splice(activeHand+1,0,second);drawTo(hand.cards);drawTo(second.cards);if(hand.cards[0].rank==='A'){hand.done=true;second.done=true;}haptic('medium');render();if(hand.done)return nextHand();return true;}
  ...
  function settle(){
    ...
    phase=STATES.PRESENTING_RESULT;const labels={BLACKJACK:'BLACKJACK!',WIN:'KAZANDIN',PUSH:'PUSH',BUST:'BUST',DEALER:'DEALER WIN'},result=root.querySelector('[data-blackjack-result]');result.className=`blackjack-result show ${best.toLowerCase()}`;result.innerHTML=`<small>MARINO LOUNGE</small><strong>${labels[best]}</strong><span>${totalPayout} DEMO JETON</span>`;
    window.MarinoAudio?.play?.(best==='BLACKJACK'?'blackjack_win':best==='WIN'?'normal_win':best==='PUSH'?'push':best==='BUST'?'bust':'ui_close');haptic(best==='BLACKJACK'?'heavy':best==='WIN'?'medium':'light');history.unshift(`${labels[best]} • ${totalPayout} demo`);history=history.slice(0,10);window.MarinoGameShell?.refreshWallet?.();render();schedule(resetRound,900);
  }
  function onClick(event){
    ...
    if(action==='free'&&[STATES.IDLE,STATES.BETTING].includes(phase)){if((window.MarinoDemoWallet?.snapshot?.().freeBets||0)>0){freeBetArmed=true;message('Demo Free Bet hazır.');}else message('Kullanılabilir demo Free Bet yok.');}
  ...
  const game={name:'Marino Blackjack Lounge',mount:markup,isActive:()=>![STATES.IDLE,STATES.BETTING,STATES.PRESENTING_RESULT].includes(phase),setPaused,onOpen:()=>{engine=createEngine(`blackjack-${Date.now()}`);phase=STATES.IDLE;paused=false;render();},onClose:()=>{clearTimers();paused=false;phase=STATES.IDLE;},history:()=>history.slice(),rules:'<p>6 deste kullanılır. Dealer soft 17’de kart çeker. Natural Blackjack 3:2, normal kazanç 1:1, Push bahis iadesidir.</p><p>Insurance ve surrender yoktur. En fazla üç split ile dört el oynanabilir. Split aslara yalnız bir kart verilir. Tüm jeton ve haklar sanal demodur.</p>'};
